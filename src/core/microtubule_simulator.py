...
class MicrotubuleLattice:
    ...
    def _create_adjacency_matrix(self) -> sparse.csr_matrix:
        ...
        adjacency = sparse.csr_matrix(
            (data, (rows, cols)), shape=(self.num_tubulins, self.num_tubulins)
        )
...
class QuantumCoherenceSimulator:
    ...
    def _calculate_decoherence_rates(self) -> Dict[str, float]:
        ...
        gamma_thermal = (
            ...
            * self.config.superposition_separation_m**2
        ) / (2 * self.hbar**2)
        ...
        gamma_viscous = (
            6
            * np.pi
            * self.config.viscosity_cp
            * 1e-3  # Convert to Pa·s
            * self.config.tubulin_spacing_nm
            * 1e-9
            * self.config.superposition_separation_m**2
        ) / self.hbar
        ...
        gamma_em = (
            self.config.dielectic_constant
            * (1000 * 3.33564e-28) ** 2  # Dipole moment squared
            * self.config.superposition_separation_m**2
        ) / (
            self.hbar * (4 * np.pi * 8.854e-12 * (10e-9) ** 3)
        )  # 10 nm distance
...
class MicrotubuleSimulator:
    ...
    def _simulation_step(self, step: int):
        ...
        collapse_probs = self.coherence_sim.update_coherence(self.lattice, self.config.time_step_s)
    ...
    def _analyze_collapse_patterns(self) -> Dict[str, Any]:
        ...
        if collapse_times:
            ...
            stats = {
                ...
                "regularity_index": (
                    np.std(collapse_times) / np.mean(collapse_times)
                    if np.mean(collapse_times) > 0
                    else 0
                ),
            }
        else:
    ...
    for i in numba.prange(n):
        ...
        for j in range(n):
            ...
            if r > 0:
                r_hat = r_vec / r
                p_dot_r = (
                    dipoles[j, 0] * r_hat[0] + dipoles[j, 1] * r_hat[1] + dipoles[j, 2] * r_hat[2]
                )
