...
class NeuralVisualizer:
    ...
    def animate_activity(
        self, activity: np.ndarray, interval: int = 50, save_path: Optional[str] = None
        ...
        # Initial frame
        im = ax.imshow(
            reshaped_activity[:, :, 0], cmap="viridis", vmin=activity.min(), vmax=activity.max()
        )
        plt.colorbar(im, ax=ax, label="Activity")
        ...
        anim = animation.FuncAnimation(
            fig, update, frames=activity.shape[1], interval=interval, blit=True
        )
    ...
    def plot_phi_dynamics(
        ...
        if collapse_events is not None:
            ax.scatter(
                collapse_events,
                phi_values[collapse_events.astype(int)],
                color="red",
                s=50,
                marker="x",
