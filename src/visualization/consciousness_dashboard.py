    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "current": {
                ...
                "collapse_rate": (
                    self.metrics["collapse_rate"][-1] if self.metrics["collapse_rate"] else 0.0
                ),
                "consciousness_level": (
                    self.metrics["consciousness_level"][-1]
                    if self.metrics["consciousness_level"]
                    else 0.0
                ),
            },
            "stats": {
                ...
                "collapse_rate_mean": (
                    df["collapse_rate"].mean() if "collapse_rate" in df.columns else 0.0
                ),
            },
            "alerts": {
            ...
            "timestamps": {
                ...
                "duration": (
                    (self.timestamps[-1] - self.timestamps[0]) if len(self.timestamps) >= 2 else 0.0
                ),
            },
        }
...
class ConsciousnessDashboard:
    ...
    def __init__(
        ...
        # Initialize Dash app
        self.app = dash.Dash(__name__, external_stylesheets=[dbc.themes.DARKLY], title=title)
    ...
    def _create_layout(self) -> html.Div:
        ...
        return html.Div(
            [
                ...
                dcc.Interval(
                    id="interval-component", interval=1000, n_intervals=0  # Update every second
                ),
            ]
        )
                        ...
                        dbc.Card(
                            [
                                dbc.CardBody(
                                    [
                                        html.H5(
                                            "Integrated Information (Φ)", className="card-title"
                                        ),
                                        html.H2(
                                            "0.000",
                                            id="phi-value",
                            ...
                            [
                                dbc.CardBody(
                                    [
                                        ...
                                        html.H2(
                                            "N/A",
                                            id="consciousness-level-value",
                                            className="card-text text-center",
                                        ),
                                        html.P(
                                            "Status",
                                            id="consciousness-level-status",
                                            className="card-text text-center",
                                        ),
                                    ]
                                ),
