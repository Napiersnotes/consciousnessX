...
class ConsciousnessXUI:
                    ...
                    with st.spinner("Searching..."):
                        try:
                            ...
                            for result in results:
                                with st.expander(
                                    f"{result['memory_type']} - Score: {result['score']:.3f}"
                                ):
    ...
    def render_tools_interface(self):
        ...
        # Get available tools
        try:
            tools = asyncio.run(self.make_api_request(endpoint="/api/v1/tools", method="GET"))
                ...
                with st.expander(f"🔧 {tool['name']}"):
                    ...
                    for param_name, param_spec in params.items():
                        ...
                        if param_type == "string":
                            value = st.text_input(
                                f"{param_name}", help=param_spec.get("description", "")
                            )
    ...
    def run(self):
        ...
        # Main content area
        tab1, tab2, tab3, tab4 = st.tabs(["💬 Chat", "📊 Dashboard", "🧠 Memory", "🛠️ Tools"])
