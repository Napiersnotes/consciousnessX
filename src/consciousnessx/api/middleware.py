...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Log response
        logger.info(f"Response: {method} {url} - {response.status_code} " f"({latency:.3f}s)")
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Check limits
        if (
            int(minute_count) >= settings.RATE_LIMIT_PER_MINUTE
            or int(hour_count) >= settings.RATE_LIMIT_PER_HOUR
        ):
