...
class HigherOrderThought:
    ...
    def __init__(self, config: Optional[HOTConfig] = None) -> None:
        ...
        self._first_order_matrix: np.ndarray = np.zeros(self.config.num_first_order * 
                                                        self.config.first_order_dim)
        
        # Transformation matrices
        self._first_to_second = self._initialize_transformation(
            self.config.first_order_dim, 
            self.config.second_order_dim,
            self.config.num_first_order
        )
        self._second_to_third = self._initialize_transformation(
            self.config.second_order_dim,
            self.config.third_order_dim,
            self.config.num_second_order
        )
    ...
    def _initialize_transformation(self, input_dim: int, output_dim: int, 
                                   num_inputs: int) -> np.ndarray:
    ...
    def add_first_order_state(self, state_id: str, content: np.ndarray) -> None:
        ...
        representation = MetaRepresentation(
            level=HOTLevel.FIRST_ORDER,
            content=content.copy(),
            source_representation=state_id
        )
    ...
    def _form_meta_awareness(self) -> MetaRepresentation:
        ...
        if not self._third_order_states:
            ...
            return MetaRepresentation(
                level=HOTLevel.META,
                content=np.zeros(self.config.third_order_dim)
            )
    ...
    def compute_consciousness(self) -> float:
        ...
        integration = np.mean([
            np.mean(np.abs(self._first_to_second)),
            np.mean(np.abs(self._second_to_third))
        ])
        ...
        consciousness = (0.4 * meta_strength + 
                        0.3 * integration + 
                        0.3 * coherence)
    ...
    def reset(self) -> None:
        ...
        self._first_order_matrix = np.zeros(self.config.num_first_order * 
                                           self.config.first_order_dim)
