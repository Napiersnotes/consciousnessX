...
class GlobalWorkspaceTheory:
    ...
    def register_module(self, module_id: str, module_type: WorkspaceModuleType, size: int) -> None:
    ...
    def compute_competition(self) -> Dict[str, float]:
        ...
        for module_id, module in self._modules.items():
            ...
            recent_broadcasts = [
                b for b in self._broadcast_history[-10:] if b.source_module == module_id
            ]
            novelty = 1.0 / (1.0 + len(recent_broadcasts))

            # Combined score
            score = activation + attention_boost * self.config.priority_weight + novelty * 0.1
    ...
    def broadcast_conscious(self) -> Optional[WorkspaceBroadcast]:
        ...
        logger.info(
            f"Broadcast from {winner}: score={winner_score:.3f}, " f"recipients={len(recipients)}"
        )
    ...
    def get_cooperation_matrix(self) -> np.ndarray:
        ...
        for i, mid1 in enumerate(module_ids):
            for j, mid2 in enumerate(module_ids):
                ...
                broadcasts_to_j = sum(
                    1
                    for b in self._broadcast_history
    ...
    def get_consciousness_metrics(self) -> Dict[str, float]:
        ...
        if np.sum(self._attention_distribution) > 0:
            attention_entropy = -np.sum(
                self._attention_distribution * np.log2(self._attention_distribution + 1e-10)
            )
