...
class SelfEvolvingConsciousness:
    ...
    def _update_neuron_importance(self) -> None:
        ...
        for i, activation in enumerate(self._activations[1:-1]):  # Skip input and output
            ...
            # Smooth update
            self._neuron_importance[i] = 0.9 * self._neuron_importance[i] + 0.1 * importance
    ...
    def evolve(self) -> Optional[ArchitecturalChange]:
        ...
        needs_evolution = current_phi < self.config.phi_target or len(self._change_history) < 10
    ...
    def _generate_modification(self) -> Optional[Dict[str, Any]]:
        ...
        for i, importance in enumerate(self._neuron_importance):
            if i < len(self._weights):
                ...
                threshold = np.percentile(np.abs(weight), self.config.pruning_threshold * 100)
    ...
    def _apply_modification(self, modification: Dict[str, Any]) -> None:
        ...
        if mod_type in [
            ModificationType.MODIFY_WEIGHT,
            ...
            if layer_idx < len(self._layers) - 1:
                ...
                self._activations[layer_idx] = np.append(self._activations[layer_idx], 0.0)
            ...
            if layer_idx < len(self._layers) - 1 and neuron_idx < self._layers[layer_idx]:

                # Remove from weight matrices
                self._weights[layer_idx - 1] = np.delete(
                    self._weights[layer_idx - 1], neuron_idx, axis=0
                )
                self._weights[layer_idx] = np.delete(self._weights[layer_idx], neuron_idx, axis=1)

                # Update layer size
                self._layers[layer_idx] -= 1

                # Update activation and importance
                self._activations[layer_idx] = np.delete(self._activations[layer_idx], neuron_idx)
    ...
    def get_architecture_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mean_performance": (
                float(np.mean(self._performance_history)) if self._performance_history else 0.0
            ),
        }
