...
class OrchORLayer:
    ...
    def __init__(self, layer_id: int, config: Optional[OrchORConfig] = None) -> None:
        ...
        self._entanglement_matrix = np.zeros((self.config.num_neurons, self.config.num_neurons))
        ...
        logger.info(
            f"Initialized OrchORLayer {layer_id} with " f"{self.config.num_neurons} neurons"
        )
    ...
    def _compute_activations(self, potentials: np.ndarray) -> np.ndarray:
        ...
        # Thresholding
        activations = np.where(activations > self.config.activation_threshold, activations, 0.0)
    ...
    def _update_phi_values(self) -> None:
        ...
        activation_entropy = -np.sum(self._activations * np.log2(self._activations + 1e-10))
    ...
    def _check_for_collapse(self) -> Optional[CollapseEvent]:
        ...
        if np.random.random() < self.config.collapse_probability:
            ...
            logger.info(
                f"Collapse detected in layer {self.layer_id}: "
