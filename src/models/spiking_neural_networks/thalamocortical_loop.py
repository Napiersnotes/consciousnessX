...
class ThalamocorticalLoop:
    ...
    def __init__(self, loop_id: int, config: Optional[ThalamocorticalConfig] = None) -> None:
    ...
    def _initialize_trn(self) -> None:
        ...
        for i in range(self.config.num_trn_units):
            ...
            neuron_type = (
                TRNNeuronType.CORE if i < self.config.num_trn_units // 2 else TRNNeuronType.MATRIX
            )

            neuron = TRNNeuron(neuron_id=i, neuron_type=neuron_type)
    ...
    def _initialize_thalamocortical(self) -> np.ndarray:
        ...
        for i in range(n_thal):
            ...
            for j in range(n_cort):
                ...
                weights[i, j] = (
                    np.exp(-((distance / width) ** 2)) * self.config.thalamocortical_strength
                )
    ...
    def process(
        self, sensory_input: np.ndarray, cortical_feedback: Optional[np.ndarray] = None
    ) -> np.ndarray:
    ...
    def _update_thalamic_units(self, corticothalamic_input: np.ndarray) -> None:
        ...
        total_input = (self._sensory_input + corticothalamic_input) * self._attentional_gain
