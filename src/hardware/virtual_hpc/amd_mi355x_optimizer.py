...
logging.basicConfig(
    level=logging.INFO,
    format=(
        "%(asctime)s.%(msecs)03d [%(levelname)s] %(name)s: %(message)s [%(metric_name)s=%(metric_value)s]"
        if "%(metric_name)s" in locals()
        else "%(asctime)s.%(msecs)03d [%(levelname)s] %(name)s: %(message)s"
    ),
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
...
class KernelOptimizationConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if total_threads > self.max_workgroup_size:
            warnings.append(
                f"Workgroup size {total_threads} exceeds maximum {self.max_workgroup_size}"
            )

        if total_threads % self.wavefront_size != 0:
            warnings.append(
                f"Workgroup size {total_threads} not divisible by wavefront size {self.wavefront_size}"
            )

        # Check shared memory
        if self.shared_memory_bytes > self.max_shared_memory:
            warnings.append(
                f"Shared memory {self.shared_memory_bytes}B exceeds maximum {self.max_shared_memory}B"
            )
