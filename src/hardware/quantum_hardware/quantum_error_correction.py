...
class SurfaceCode:
        ...
        for i in range(len(state)):
            if np.random.random() < error_prob:
                error_type = np.random.choice(
                    [ErrorType.X_ERROR, ErrorType.Z_ERROR, ErrorType.Y_ERROR]
                )
    ...
    def _calculate_parity(
        self, state: np.ndarray, position: Tuple[int, int], stabilizer_type: str
    ) -> int:
    ...
    def decode_syndrome(
        self, syndrome: Dict[Tuple[int, int], int]
    ) -> List[Tuple[int, int, ErrorType]]:
    ...
    def correct_errors(self, state: np.ndarray) -> np.ndarray:
        ...
        for qubit_pos, error_type in errors:
            if error_type == ErrorType.X_ERROR or error_type == ErrorType.Y_ERROR:
                corrected_state = self._apply_x_error(
                    corrected_state, self._data_qubits.index(qubit_pos)
                )
            if error_type == ErrorType.Z_ERROR or error_type == ErrorType.Y_ERROR:
                corrected_state = self._apply_z_error(
                    corrected_state, self._data_qubits.index(qubit_pos)
                )
