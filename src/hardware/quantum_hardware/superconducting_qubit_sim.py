...
class TransmonQubit:
    ...
    def apply_arbitrary_rotation(self, theta: float, phi: float) -> None:
        ...
        rotation = (np.cos(theta / 2) * np.eye(2) - 
                    1j * np.sin(theta / 2) * 
...
class SuperconductingQubit(TransmonQubit):
    ...
    def __init__(self, config: Optional[QubitConfig] = None, 
                 num_levels: int = 3) -> None:
    ...
    def get_anharmonicity_spectrum(self) -> np.ndarray:
        ...
        frequencies = (self.config.frequency + 
                      self.config.anharmonicity * levels * (levels - 1) / 2)
        return frequencies