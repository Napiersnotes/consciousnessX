...
class TestMicrotubuleLattice(unittest.TestCase):
    ...
    def setUp(self):
        self.config = MicrotubuleConfig(
            num_protofilaments=3, num_tubulins_per_filament=5  # Small for testing
        )
...
class TestMicrotubuleSimulator(unittest.TestCase):
    ...
    def test_full_simulation(self):
        ...
        self.assertAlmostEqual(
            time_array[-1], self.config.simulation_duration_s, delta=self.config.time_step_s * 2
        )
...
class TestConsciousnessAssessment(unittest.TestCase):
    ...
    def test_analyze_collapse_patterns(self):
        ...
        # Should have expected keys
        expected_keys = [
            "mean_collapse_interval",
            "std_collapse_interval",
            "total_collapses",
            "collapse_rate",
            "regularity_index",
        ]
...
@unittest.skipUnless(os.getenv("RUN_PERFORMANCE_TESTS"), "Performance tests disabled by default")
class TestPerformance(unittest.TestCase):
    ...
    def test_large_simulation_performance(self):
        ...
        self.assertLess(
            elapsed, max_time, f"Simulation took {elapsed:.2f}s, expected < {max_time}s"
        )
        ...
        self.assertGreater(real_time_factor, 1.0, f"Real-time factor {real_time_factor:.2f} <= 1.0")
